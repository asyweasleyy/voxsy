...
type Props = {
  navigation: StackNavigationProp<MainStackParamList, 'EnstrümanSecim'>;
};
...
function InstrumentCard({
  ...
  useEffect(() => {
    Animated.parallel([
      ...
      Animated.spring(check, { toValue: selected ? 1 : 0, useNativeDriver: true, tension: 220, friction: 10 }),
    ]).start();
  }, [selected]);

  const borderColor = glow.interpolate({ inputRange: [0, 1], outputRange: [C.goldBorder, C.goldBorderSel] });
  const bgColor = glow.interpolate({ inputRange: [0, 1], outputRange: ['rgba(255,255,255,0.04)', C.goldBg] });
  const checkScale = check.interpolate({ inputRange: [0, 0.6, 1], outputRange: [0, 1.25, 1] });

  const onPressIn = () => Animated.spring(scale, { toValue: 0.92, useNativeDriver: true, tension: 300, friction: 10 }).start();
  ...
  return (
    ...
    >
      <Animated.View style={[styles.card, { transform: [{ scale }] }]}>
        <Animated.View style={[StyleSheet.absoluteFill, styles.cardInner, { borderColor, backgroundColor: bgColor }]}>
          <Animated.View style={[styles.checkBadge, { transform: [{ scale: checkScale }], opacity: check }]}>
            <Text style={styles.checkMark}>✓</Text>
          </Animated.View>
          <Text style={styles.cardEmoji}>{item.emoji}</Text>
          <Text style={[styles.cardName, selected && styles.cardNameSel]} numberOfLines={1}>
            {item.name}
          </Text>
        </Animated.View>
      </Animated.View>
...
export default function EnstrümanSecimScreen({ navigation }: Props) {
  const { user } = useAuth();
  ...
  // fetch instruments + existing selections
  useEffect(() => {
    if (!user) return;
    let cancelled = false;
    ...
    const load = async () => {
      ...
      try {
        ...
        if (cancelled) return;
        setInstruments(all);
        setSelected(new Set(current.map((i) => i.id)));
      } catch (err: any) {
        if (!cancelled) setError('Enstrümanlar yüklenemedi. Lütfen tekrar dene.');
      } finally {
      ...
      <Animated.View style={[styles.footer, { opacity: fadeAnim }]}>
        ...
        >
          {saving
            ...
            : <Text style={[styles.devamBtnText, !canProceed && styles.devamBtnTextDisabled]}>Devam Et</Text>}
        </TouchableOpacity>
      </Animated.View>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  // header
  header: { alignItems: 'center', paddingTop: Platform.OS === 'android' ? 28 : 16, paddingBottom: 20 },
  vinylRow: { width: 64, height: 64, alignItems: 'center', justifyContent: 'center', marginBottom: 24 },
  noteIcon: { fontSize: 22, color: C.gold },
  title: { fontSize: 26, fontFamily: SERIF, color: C.text, letterSpacing: 0.3, marginBottom: 10, textAlign: 'center' },
  subtitle: { fontSize: 13, color: C.textMuted, textAlign: 'center', lineHeight: 19, paddingHorizontal: 24, marginBottom: 14 },
  badge: {
  ...
  divider: { width: 36, height: 1.5, backgroundColor: C.goldMuted, borderRadius: 1, marginTop: 10, marginBottom: 4 },

  // card
  card: { width: CARD_W, height: 106 },
  cardInner: {
    flex: 1,
    borderRadius: 14,
    borderWidth: 1.5,
  ...
  cardName: { fontSize: 11, fontWeight: '600', color: C.textMuted, textAlign: 'center', letterSpacing: 0.3, textTransform: 'uppercase' },
  ...
  devamBtnText: { color: C.bg, fontSize: 15, fontWeight: '700', letterSpacing: 1.1 },
